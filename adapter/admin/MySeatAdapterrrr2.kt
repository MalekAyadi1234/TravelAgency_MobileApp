...
import com.example.retrofitdemo.admin.getwhoActivity
...
class MySeatAdapterrrr2(iddd:String, dated:String): RecyclerView.Adapter<MySeatAdapterrrr2.MyViewHolder>(){
    var idd=iddd
    var datee=dated
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if (myList[position].disponibility){
            ...
            holder.itemView.setOnClickListener { v ->
                val intent = Intent(v.context, getwhoActivity::class.java).apply {
                    putExtra("id", myList[position].id)
                    putExtra("num", myList[position].chaise.numChaise)
